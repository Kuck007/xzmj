...
struct CustomerView: View {
    ...
    private var allRecharges: [RechargeRecord] { appCore.rechargesByRechargeAtDesc }
    ...
    private var rechargedByCustomer: [UUID: Double] { appCore.rechargedByCustomer }
    private var walletByCustomer: [UUID: Double] { appCore.walletByCustomer }
    private var totalSpentByCustomer: [UUID: Double] { appCore.totalSpentByCustomer }
    private var lastVisitByCustomer: [UUID: Date] { appCore.lastVisitByCustomer }
    ...
    private var filtered: [Customer] {
        // 直接读 AppCore 预排序结果，避免每次 body 重排
        var result = appCore.customersSortedByPinyin
    ...
    var body: some View {
        ...
        VStack(spacing: 0) {
            ...
            .sheet(isPresented: $showingAdd) {
                CustomerFormView { appCore.insert($0) }
                
            }
            .sheet(isPresented: $showingRecharge) {
                RechargeSheet(customers: appCore.customers) { customer, amount, bonus, paymentMethod, note, rechargeAt in
            ...
            .alert("删除客户？", isPresented: Binding(
                ...
                Button("删除", role: .destructive) {
                    if let c = pendingDelete { appCore.delete(c) }
                }
...
struct CustomerDetailSheet: View {
    ...
    // 按当前客户过滤
    private var records: [NailServiceRecord] {
        appCore.records.filter { $0.customerId == customer.id }
            .sorted { $0.serviceDate > $1.serviceDate }
    }
    private var orders: [Order] {
        appCore.orders.filter { $0.customerId == customer.id }
            .sorted { $0.paidAt > $1.paidAt }
    }
    private var recharges: [RechargeRecord] {
        appCore.recharges.filter { $0.customerId == customer.id }
            .sorted { $0.rechargeAt > $1.rechargeAt }
    }
